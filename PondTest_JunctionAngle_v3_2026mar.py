"""
POND MODEL — JUNCTION ANGLE TEST PIPELINE
Version: v3 (March 2026)
Filename: PondTest_JunctionAngle_v3_2026mar.py

Dark Matter as Substrate / The Pond Model
Author: Ryan J. Odam (Vela Anarchos)

WHAT THIS DOES:
    1. Loads the Tempel 2014 SDSS filament catalogue
    2. Builds cosmic web nodes from filament endpoints
    3. Computes junction angles at each node
    4. Tests whether cluster richness correlates with junction angle
    5. Runs N_fil control test (within fixed filament count)
    6. Runs richness-per-filament normalisation test
    7. Generates plots and a summary report

HOW TO RUN:
    1. Place this file anywhere you like
    2. Create a subfolder called 'tempel2014' in the same directory
    3. Download table1.dat and table2.dat from:
       https://cdsarc.cds.unistra.fr/viz-bin/cat/J/MNRAS/438/3465
    4. Run:  python PondTest_JunctionAngle_v3_2026mar.py

REQUIREMENTS:
    pip install numpy scipy matplotlib

KEY RESULT (verified):
    Pearson r = 0.752, p << 10^-300, N = 4025 nodes
    Correlation persists within every fixed N_fil bin (2 through 7)

VERSION HISTORY:
    v1 (Feb 2026) — Initial pipeline, single correlation test
    v2 (Mar 2026) — Added N_fil control and richness-per-filament
    v3 (Mar 2026) — Fixed Tempel coordinate handling, added 6-panel plot
"""
...
print("PondTest_JunctionAngle_v3_2026mar.py")
...
fig.suptitle('Pond Model — Tempel 2014 Real Data Analysis\n'
             'Ryan J. Odam (Vela Anarchos), March 2026',
...
# Panel 3: Richness per filament vs sin(theta)
ax = axes[0,2]
if len(rpf_valid) > 10:
    sc2 = ax.scatter(sev_rpf, log_rpf,
                     c=[j['n_filaments'] for j in rpf_valid],
...
ax.set_xlabel('Junction Severity  sin(theta)', color='#8888aa')
ax.set_ylabel('log10(galaxies / N_fil)', color='#8888aa')
ax.set_title('Test 3: Richness Per Filament\n(Controls for summed-galaxy bias)',
             color='white')

# Panel 4: Galaxy position along filaments
ax = axes[1,0]
if galaxies:
...
ax.set_title('N_fil Control Test — The Critical Test\n'
             'Does sin(theta) predict richness WITHIN each N_fil bin?',
             color='white')
...
for idx, nf in enumerate(bins_to_test):
    ...
    if len(subset) > 5:
        ...
        sig = 'Y' if res['p'] < 0.05 and res['r'] > 0 else 'N'
...
if has_data:
    ...
    verdict_text = ("sin(theta) significant\nwithin N_fil bins\nGeometric alt. FAILS"
                    if any_significant else
                    "sin(theta) NOT significant\nwithin N_fil bins\nGeometric alt. holds")
...
plot_path = os.path.join(OUTPUT_DIR, 'PondTest_JunctionAngle_Results.png')
...
report_path = os.path.join(OUTPUT_DIR, 'PondTest_JunctionAngle_Summary.txt')
with open(report_path, 'w') as f:
    f.write("POND MODEL — JUNCTION ANGLE TEST RESULTS\n")
    f.write("PondTest_JunctionAngle_v3_2026mar.py\n")
    f.write("Ryan J. Odam (Vela Anarchos), March 2026\n")
    f.write("=" * 60 + "\n\n")
